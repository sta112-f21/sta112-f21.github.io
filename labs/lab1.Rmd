...
**Due:** Friday, September 3, 12:00pm (noon) on Canvas (see the end of the lab for submission instructions)
...
**Step 1:** Download the Lab 1 template file: [lab_01_template.Rmd](https://sta112-f21.github.io/labs/lab_01_template.Rmd). Save it somewhere on your computer that you can find it easily (e.g., a folder on your desktop).
...
`mean` is a function for calculating the mean, and `pull` extracts a column from the dataset. So `penguins %>% pull(body_mass_g) %>% mean()` takes `penguins`, extracts the body mass column, and then calculates the mean. Notice that pipes can be chained together!
:::
...
`summarize` calculates summary statistics for a dataset. In `summarize`, we specify which statistics to calculate, and on which columns. `group_by` is often used before `summarize`, and it means "group the data by the values of a column". So, the code above means "take penguins_no_nas, group the penguins by their species, and within each group calculate the mean body mass".
:::

# Submission Instructions

When you are done with the lab, save the R Markdown file, and click the `knit` button. Make sure the resulting HTML file includes all your code and responses to the lab questions, then submit the **HTML file** on Canvas (Assignments -> Lab 1).
...
| **Total** | **100pts** |
| --------- | ---------- |
| Filename lab_01_your_name.html | 5 pts |
| Changed `author` in YAML | 5 pts |
| Exercises 1--9 | 10 pts each|