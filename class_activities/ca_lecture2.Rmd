---
title: "Class Activity, August 25"
output: 
  tufte::tufte_html:
    css: "lab.css"
...
Fill in the `...` in the following code so that the points in the scatterplot are colored by penguin species:
```
penguins %>%
  ggplot(aes(x = bill_length_mm, y = bill_depth_mm, color = ...)) +
  geom_point()
```
...
8. **Challenge:** Come up with a rule to predict penguin species, given bill length and bill depth. This will be a function $f(length, depth)$, which outputs Adelie, Chinstrap, or Gentoo, depending on the values of length and depth.