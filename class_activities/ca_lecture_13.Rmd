...
This activity investigates the relationship between the wing length and weight of sparrows on Kent island. The data are contained in the `Sparrows` dataset from the `Stat2Data` package. The activity template contains copies of the code and questions below. Run the setup chunk in the template before you begin.
...
6. Now let's check the p-value manually. Fill in the code below to calculate the p-value using the `pt` function. Confirm that it agrees with exercise 5. (Note: R just reports `< 2e-16` when p-values are below $2 \times 10^{-16}$).
